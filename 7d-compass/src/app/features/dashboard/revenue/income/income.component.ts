...
export class IncomeComponent {
  ...
  onEditTicket(ticket: any): void {
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        const index = this.ticketData.findIndex(
          (t) => t.ticketnum === ticket.ticketnum
        );
        if (index !== -1) {
          const updated = {
          ...
          // Recalcular total
          updated.total = (
            Number(updated.mcost || 0) +
            Number(updated.wcost || 0) +
            Number(updated.ecost || 0)
          ).toString();
  ...
  onDeleteTicket(ticket: any): void {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.ticketData = this.ticketData.filter(
          (t) => t.ticketnum !== ticket.ticketnum
        );
  ...
  invoiceColumns: ColumnDefinition[] = [
    ...
    },
    {
      ...
      cell: (ticket: any) => {
        ...
        const html = `<span style="color:${color}; font-weight: bold;">${sign}$${Math.abs(
          diff
        )}</span>`;
